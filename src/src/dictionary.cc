...
void Dictionary::computeSubwords(const std::string& word,
  ...
  for (size_t i = 0; i < word.size(); i++) {
    ...
    if (word[i] == args_->endjamo[0]) continue;
    std::string scgram;
    for (size_t j = i, n = 1, num_jamo = 0; j < word.size() && n <= args_->maxsc; n++) {
      
      scgram.push_back(word[j++]);
      while (j < word.size() && (word[j] & 0xC0) == 0x80) {
        scgram.push_back(word[j++]);
      }
      while (word[j]==args_->endjamo[0]){
        j++;
      }
      
      if (n >= args_->minsc && !(n == 1 && (i == 0 || j == word.size()))) {
        int32_t h = hash(scgram) % args_->bucket;
        pushHash(ngrams, h);
        substrings.push_back(scgram);
      }
    }

    std::string jamogram;
    if ((i==1) | (word[i-1] == args_->endjamo[0])) {
      for (size_t j = i, n = 1, num_jamo = 0; j < word.size(); n++) {
        ...
        if (n > args_->maxsc && num_jamo >= args_->minjn && num_jamo <= args_->maxjn && word[j-1] == args_->endjamo[0] && !(i == 0 || j == word.size())) {
    ...
    std::string chargram;
    if (i==1 || (word[i-1] == args_->endchar[0])) {
      for (size_t j = i, n = 1, num_char = 0, num_jamo = 0; j < word.size(); n++) {
        if (word[j]==args_->endchar[0]){
          num_char++;
        }
        ...
        if (n > args_->maxsc && num_jamo > args_->maxjn  && num_char >= args_->minn && num_char <= args_->maxn && 
        word[j-1] == args_->endchar[0] && !(i == 0 || j == word.size())) {
