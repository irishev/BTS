...
void analogy_test(const std::vector<std::string>& args) {
  if (args.size() < 4 || args.size() > 5) {
    printTestUsage();
    exit(EXIT_FAILURE);
  }
  ...
  std::string infile = args[3];
  if (infile == "-") {
    fasttext.analogy_test(std::cin, k);
  } else {
    std::ifstream ifs(infile);
    if (!ifs.is_open()) {
      std::cerr << "Test file cannot be opened!" << std::endl;
    ...
    fasttext.analogy_test(ifs, k);
    ifs.close();
  }
  exit(0);
}

void extract_vec(const std::vector<std::string>& args) {
  if (args.size() < 4 || args.size() > 5) {
    printTestUsage();
    exit(EXIT_FAILURE);
  }
  ...
  std::string infile = args[3];
  if (infile == "-") {
    fasttext.extract_vec(std::cin, args[2]);
  } else {
    std::ifstream ifs(infile);
    if (!ifs.is_open()) {
      std::cerr << "Test file cannot be opened!" << std::endl;
    ...
    fasttext.extract_vec(ifs, args[2]);
    ifs.close();
  }
  exit(0);
}
...
void get_analogy_vecs(const std::vector<std::string>& args) {
  ...
  std::string infile = args[3];
  if (infile == "-") {
    fasttext.get_analogy_vecs(std::cin, args[4]);
  } else {
    std::ifstream ifs(infile);
    if (!ifs.is_open()) {
      std::cerr << "Test file cannot be opened!" << std::endl;
    ...
    fasttext.get_analogy_vecs(ifs, args[4]);
    ifs.close();
  }
  exit(0);
}
...
int main(int argc, char** argv) {
  ...
  if (command == "skipgram" || command == "cbow" || command == "supervised") {
    ...
    predict(args);
  } else if (command == "analogy_vecs") {
    get_analogy_vecs(args);
  } else {
    printUsage();
    exit(EXIT_FAILURE);
  }
